...
def process_handwriting(image_path):
    ...
    # 4. Process each character (with all  validation checks)
    vis_img = img.copy()
    ...
    for i in range(n_boxes):
        ...
        for j, char in enumerate(text):
            if not char.isalnum():  # Skip non-alphanumeric (matches your 'continue' logic)
                continue
            ...
            if char_img.size > 0:
                ...
                if hasattr(result, 'probs') and result.probs.top1 == 0:  # Assuming class 0 is dyslexic
                    # Draw rectangle (matches your highlighting approach)
